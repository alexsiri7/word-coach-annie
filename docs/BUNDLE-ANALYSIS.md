...
| harper-wasm | _(WASM chunk)_ | ~1–2 MB | Spell/grammar linting first use |

> **Note**: harper-wasm loads a ~1–2 MB WASM binary on first use of the linting feature. It is dynamically imported in `src/lib/linting/harper-linter.ts` and does not affect page load unless the linter is invoked.
