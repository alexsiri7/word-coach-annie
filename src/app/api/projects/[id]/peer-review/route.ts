...
export async function POST(
  ...
  try {
    ...
    // Run all three reviewer agents in parallel
      ...
      client.chat.completions
        .create({
          ...
          messages: [
            ...
            { role: "user", content: persona.userPrompt(project.title, manuscript) },
          ],
          max_tokens: 700,
          temperature: 0.7,
        })
    ...
    const consensusRes = await client.chat.completions.create({
      model: aiConfig.model,
      messages: [
        {
          role: "system",
        ...
        { role: "user", content: buildConsensusPrompt(project.title, reviews) },
      ],
      max_tokens: 500,
      temperature: 0.5,
    });

    const consensus = consensusRes.choices[0]?.message?.content?.trim() || "";
