...
export async function GET(request: NextRequest) {
    ...
    const stateBaseUrl = env.GOOGLE_REDIRECT_URI
        ? new URL(env.GOOGLE_REDIRECT_URI).origin
        : request.nextUrl.origin;
    if (!stateParam || !stateCookie || stateParam !== stateCookie) {
        return NextResponse.redirect(
            new URL("/login?error=invalid_state", stateBaseUrl)
        );
    }
    // Verify HMAC binding: state must be nonce.sig generated by this server
    const dotIdx = stateParam.indexOf(".");
    const nonce = dotIdx !== -1 ? stateParam.slice(0, dotIdx) : null;
    const sig = dotIdx !== -1 ? stateParam.slice(dotIdx + 1) : null;
    if (!nonce || !sig) {
        return NextResponse.redirect(
            new URL("/login?error=invalid_state", stateBaseUrl)
        );
